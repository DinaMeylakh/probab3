...
All masses are sampled from a present day mass function (PDMF) assuming the sampling is at the core.
"""
...
OUTPUT_FOLDER = "sampling_logs/unequal_masses_bbh/" 
...
def plot2(folder_path: str = OUTPUT_FOLDER, include_binary: bool = True, merger_only: bool = False):
    ...
    click.secho(f"Plotting Mcs", fg="green")
    if os.path.exists(f'{folder_path}Mcs_unequal_bbh_stats_data_{file_suffix}.json'):
        plotter.plot_parameters_stats_from_file(f'{folder_path}Mcs_unequal_bbh_stats_data_{file_suffix}.json', merger_only=merger_only)
        ...
        plotter.plot_few_cluster_files_stats(sample_paths_Mcs, 'Mc', include_binary=include_binary, merger_only=merger_only,
                                             save_plot_data_path=f'{folder_path}Mcs_unequal_bbh_stats_data_{file_suffix}.json')
    
    click.secho(f"Plotting cs", fg="green")
    if os.path.exists(f'{folder_path}cs_unequal_bbh_stats_data_{file_suffix}.json'):
        plotter.plot_parameters_stats_from_file(f'{folder_path}cs_unequal_bbh_stats_data_{file_suffix}.json', merger_only=merger_only) 
    else:
        ...
        plotter.plot_few_cluster_files_stats(sample_paths_rcs, 'c', include_binary=include_binary, merger_only=merger_only, 
                                         save_plot_data_path=f'{folder_path}cs_unequal_bbh_stats_data_{file_suffix}.json')

def plot_from_saved(folder_path: str = OUTPUT_FOLDER):
    plotter.plot_parameters_stats_from_file(f'{folder_path}Mcs_unequal_bbh_stats_data_{file_suffix}.json')
    plotter.plot_parameters_stats_from_file(f'{folder_path}cs_unequal_bbh_stats_data_{file_suffix}.json')
...
def plot(folder_path: str = OUTPUT_FOLDER):
    sample_paths_Mcs = glob.glob(f"{folder_path}*_rc{defualt_rc_index}_n{N}_{file_suffix}.jsonl", recursive=False)
    manual_exchange_stats.save_manual_exchange_stats(file_paths=sample_paths_Mcs, save_path=f"{folder_path}Mcs_manual_exchange_stats_{file_suffix}.json", verbose=True)

    sample_paths_rcs = glob.glob(f"{folder_path}sample_Mc{default_Mc_index}_rh{default_Mc_index}_rc*_n{N}_{file_suffix}.jsonl", recursive=False)
    manual_exchange_stats.save_manual_exchange_stats(file_paths=sample_paths_rcs, save_path=f"{folder_path}cs_manual_exchange_stats_{file_suffix}.json", verbose=True, run_param='c')
